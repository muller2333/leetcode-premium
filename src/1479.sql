...
from (select item_category,
      ...
      from (select case day
                   ...
                   item_category,
                   total
            from (select dayofweek(order_date) day,
                         item_category,
                         sum(quantity)         total
                  from Orders
                           left join Items on Items.item_id = Orders.item_id
                  group by dayofweek(order_date), item_category) mid) mid2,
           (select 'Monday'    Monday,
                   'Tuesday'   Tuesday,
                   'Wednesday' Wednesday,
                   'Thursday'  Thursday,
                   'Friday'    Friday,
...
select item_category Category,
       ...
       0             SUNDAY
from Items
where item_category not in (select distinct item_category
                            from Orders
